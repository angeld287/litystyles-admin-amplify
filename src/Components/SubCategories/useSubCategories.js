...
const useSubCategories = () => {
	...
	useEffect(() => {
		...
		const fetch = async () => {
			...
			try {
				api = await API.graphql(graphqlOperation(listSubCategorys));
			} catch (error) {
				setLoading(false);
				setError(true);
			}
			...
			if (!didCancel) {
				setList(api.data.listSubCategorys.items);
				setLoading(false);
			}
		};
	...
	const _handleDelete = async (id) => {
		...
		if (result.value) {
			try {
				await API.graphql(graphqlOperation(deleteSubCategory, { input }));
